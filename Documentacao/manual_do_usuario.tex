...
O botão de brilho na barra de título alterna entre o tema claro e o escuro. A
escolha fica guardada: o software reabre no tema que você deixou.

O padrão é o \textbf{claro}, porque é o que se enxerga num projetor. Em
laboratório com pouca luz, o escuro cansa menos a vista. Nada além das cores
muda --- os mesmos controles, nos mesmos lugares.

Em qualquer um dos dois, as cores têm significado fixo: temperatura em laranja,
fotocorrente em azul, ajuste de regressão em roxo, pontos descartados em cinza.
Verde, âmbar e vermelho ficam reservados para \emph{estado} (bom, atenção,
problema) e nunca aparecem como cor de série num gráfico. E nenhuma informação
depende só da cor: todo veredicto vem acompanhado de ícone e texto.
...
\subsection{Limite de corrente da fonte}

O campo \campo{Limite de corrente da fonte} define quanta corrente a fonte
pode entregar ao filamento. É uma proteção física: ultrapassado o limite, a
fonte segura a corrente e derruba a tensão.

\begin{aviso}
\textbf{Confirme este valor para o seu filamento antes da primeira coleta.} O
padrão é 1,5~A, conservador. O valor efetivamente usado fica gravado nos
metadados de cada coleta.
\end{aviso}

\subsection{Endereço do multímetro}

Os campos de \campo{IP} e \campo{porta} definem onde o software procura o
multímetro na rede. Se o aparelho mudar de endereço, corrija aqui e procure de
novo --- o valor fica guardado para as próximas sessões.
...
\paragraph{Perfis.}
No topo do painel há três caixas de seleção --- \campo{LED (sensor)},
\campo{Filamento} e \campo{Varredura} --- que preenchem os campos abaixo de uma
vez. Passe o mouse sobre cada caixa para ver a \emph{fonte} daqueles valores.

Os perfis ficam em arquivos de texto editáveis, em \texttt{Software/profiles/}:
para cadastrar um LED novo, basta acrescentá-lo a \texttt{leds.json}. Se algum
arquivo estiver com erro, o software avisa em amarelo no próprio painel e segue
funcionando com os valores embutidos --- uma coleta nunca é impedida por um
JSON malformado.

O botão \botao{Salvar varredura atual como perfil} guarda a faixa, o passo, a
espera e o número de leituras que você está usando, para reaproveitar depois.

\begin{aviso}
\textbf{A resistência a frio não é perfil.} Ela é propriedade do filamento
específico que está montado na sua bancada, e deve ser medida a cada montagem.
Por isso continua sendo um campo que você preenche, não algo que vem de
catálogo.
\end{aviso}
...
\textbf{Campo} & \textbf{Padrão} & \textbf{O que é} \\
\midrule
Perfil de filamento & Padrão do software &
Preenche $\alpha$ e $\beta$ com um par de valores conhecido. Passe o mouse
sobre a caixa para ver a fonte. \\[0.3em]
Resistência a frio medida & 1,2~$\Omega$ &
O valor que o ohmímetro lê no filamento em repouso. \\[0.3em]
Temperatura ambiente & 25~°C &
A temperatura em que a medida acima foi feita. \\[0.3em]
Coef. linear $\alpha$ & 5,23e-3~K$^{-1}$ & Vem do preset; editável. \\[0.3em]
Coef. quadrático $\beta$ & 7,0e-7~K$^{-2}$ & Vem do preset; editável. \\[0.3em]
Comprimento de onda $\lambda$ & 590~nm &
Comprimento de onda característico do LED sensor. \\[0.3em]
Resistência dos cabos & 0~$\Omega$ &
A medição é a dois fios, então a resistência dos cabos entra somada à do
filamento. Meça curto-circuitando as pontas e informe aqui; deixe zero se não
souber. \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{aviso}
\textbf{Por que existem dois campos para a resistência a frio.} O modelo
define $R_0$ como a resistência a \textbf{0~°C}, mas você mede o filamento na
temperatura ambiente, onde ele já está cerca de 13\% mais resistivo. O
software faz essa conversão para você --- basta informar em que temperatura a
medida foi feita. Logo abaixo dos campos, uma linha azul mostra o $R_0$
resultante, que é o valor efetivamente usado no cálculo.

Informar a temperatura ambiente errada desloca \emph{todas} as temperaturas
calculadas, e com elas o valor de $h$.
\end{aviso}

\paragraph{Escolhendo o preset de coeficientes.}
Os coeficientes $\alpha$ e $\beta$ entram direto no cálculo da temperatura. O
preset \campo{Padrão do software} traz o par que sempre foi usado --- e com o
qual todas as coletas históricas foram processadas --- mas cuja procedência não
está documentada. O preset \campo{Artigo RBEF / PHYWE} traz os valores do
artigo de referência, com fonte citada. Para medidas novas, prefira um preset
com fonte conhecida; para reprocessar dados antigos, mantenha o padrão.
...
temperaturas erradas. \\[0.3em]
Temp. mínima p/ regressão & 1800~K &
\textbf{Só entram no cálculo de $h$ os pontos acima desta temperatura.} Não
afeta a coleta: todos os pontos continuam sendo medidos e gravados. Ver
seção~\ref{sec:faixa}. \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{Tensao\_Medida\_V} & Tensão lida de volta da fonte, em volts \\
\bottomrule
\end{tabular}
\end{center}

A última coluna é a tensão realmente presente nos terminais, lida da fonte, e é
ela que o software usa para calcular a resistência --- a leitura de retorno é
mais exata que o valor programado. As duas ficam gravadas para que você possa
conferir a diferença depois. As cinco primeiras colunas mantêm a ordem
histórica, então arquivos antigos continuam legíveis pelas mesmas ferramentas.

Se você pedir mais de uma leitura por ponto, duas colunas adicionais registram
o desvio estatístico e o número de leituras.

\subsection{O arquivo de metadados}

Junto com o CSV, cada coleta grava um arquivo \texttt{.json} de mesmo nome:

\begin{quote}\ttfamily
data\_backup/exp\_planck\_20260805\_143000.csv \quad -- as medidas\\
data\_backup/exp\_planck\_20260805\_143000.json \quad -- como foram obtidas
\end{quote}

Ele guarda tudo que você configurou: os perfis escolhidos, a resistência a frio
medida \emph{e} a temperatura em que foi medida, o $R_0$ corrigido, os
coeficientes, o LED, a faixa de varredura, e ao final o resultado com incerteza
e o orçamento completo.

\begin{aviso}
\textbf{Por que isso é importante.} Meses depois, um CSV sozinho não diz com
quais parâmetros foi obtido. Com o JSON ao lado, qualquer coleta pode ser
reinterpretada, comparada ou reprocessada sem depender de memória ou de
anotação avulsa.

O arquivo é gravado logo no início da coleta e atualizado no fim --- então,
mesmo que falte energia no meio do experimento, os parâmetros já estão
registrados.
\end{aviso}
...
O painel de resultado tem três degraus, de propósito. No topo, em corpo grande e
centralizado, o \textbf{valor de $h$ com a sua incerteza} e o veredicto de
compatibilidade --- é o que se procura ao terminar uma coleta. Abaixo, quatro
\textbf{cartões de diagnóstico} do mesmo tamanho, porque nenhum deles é mais
importante que os outros. Fechando, o \textbf{orçamento de incertezas}.

Nos cartões de erro e de $R^2$ a cor acompanha o valor: verde quando está bom,
âmbar quando merece atenção, vermelho quando alguma coisa precisa ser
investigada. A cor é reforço --- o número está escrito no mesmo cartão.

Os números, um a um:

\begin{description}[style=nextline,leftmargin=0.6cm]
  \item[$h$ com sua incerteza]
    Apresentado na forma $(6{,}50 \pm 0{,}52)\times 10^{-34}$~J$\cdot$s (k=2).
    O $\pm$ é a incerteza expandida: significa, grosso modo, 95\% de confiança
    de que o valor verdadeiro está nesse intervalo. O valor de referência
    (CODATA) é $6{,}62607015 \times 10^{-34}$~J$\cdot$s.
  \item[Erro relativo]
  ...
  \item[$\chi^2$ reduzido]
    Compara a dispersão observada com a incerteza declarada dos instrumentos.
    \textbf{Neste experimento ele fica bem abaixo de 1, e isso é normal} --- a
    especificação do fabricante é um limite de pior caso, e o ruído real é
    menor. Serve para comparar coletas entre si: uma mudança brusca indica que
    algo mudou na montagem.
  \item[Pontos usados]
    Quantos dos pontos coletados entraram na conta.
  \item[Orçamento de incertezas]
    A fatia de cada fonte na incerteza final. Diz onde vale investir para
    melhorar o resultado.
\end{description}

\subsection{Compatibilidade com o valor de referência}

Ao final, o software informa se o valor da CODATA cai \emph{dentro} da
incerteza declarada.

\begin{itemize}[nosep]
  \item \textbf{Cai dentro} --- o experimento é consistente com o valor
        conhecido, dentro da precisão que a montagem permite. É o resultado
        esperado de uma coleta bem feita.
  \item \textbf{Cai fora} --- há erro sistemático não contabilizado. Os
        suspeitos habituais são a calibração ($R_0$, temperatura ambiente,
        coeficientes) e efeitos físicos que o modelo não inclui: emissividade
        do tungstênio, reflexões na montagem, alinhamento entre LED e
        filamento.
\end{itemize}

\subsection{Onde a incerteza nasce}

O orçamento quase sempre é dominado pela \textbf{largura espectral do LED}. Um
LED não responde a um único comprimento de onda, e sim a uma banda de algumas
dezenas de nanômetros --- sobre 590~nm, isso são vários por cento, mais do que
qualquer outro termo da cadeia.

\begin{aviso}
\textbf{Consequência prática.} Melhorar a eletrônica não melhora esse termo.
Se você quiser um resultado mais preciso, o caminho é um sensor espectralmente
mais seletivo (um filtro de banda estreita, por exemplo), não um multímetro
melhor. O orçamento na tela mostra isso a cada coleta.
\end{aviso}

\subsection{Repetindo leituras (incerteza Tipo A)}

O campo \campo{Leituras por ponto (N)} manda o software medir a fotocorrente
$N$ vezes em cada tensão. Com $N > 1$, ele calcula também a dispersão
estatística daquele ponto e a soma à incerteza do fabricante.

Custa tempo: $N$ leituras por tensão, e cada leitura do multímetro leva alguns
décimos de segundo. Com $N = 1$ (padrão) só existe a incerteza de datasheet.
...
\subsection{Como o software resolve isso}

\textbf{O corte é feito na conta, não na coleta.} Você varre a faixa que
quiser --- inclusive de 0 a 12~V --- e todos os pontos são medidos, mostrados e
gravados no CSV. Apenas a regressão final usa a região informativa,
determinada pelo campo \campo{Temp. mínima p/ regressão}.

Isso significa que não é preciso escolher entre registrar o comportamento
completo do filamento e obter um bom valor de $h$: dá para ter os dois.

\paragraph{O que você vê na tela.}
No gráfico de linearização, os pontos aparecem em duas cores:

\begin{itemize}[nosep]
  \item \textbf{coloridos} --- entraram na regressão;
  \item \textbf{cinza} --- foram descartados por estarem fora da região de
        Wien (ou por temperatura fisicamente impossível).
\end{itemize}

O registro em tempo real marca cada linha descartada, e o painel de status ao
final informa quantos pontos de quantos foram efetivamente usados. Essa
contagem também vai para o relatório PDF.

\paragraph{O efeito, em números.}
Uma varredura completa de 0 a 12~V com 49 pontos, na bancada simulada:

\begin{center}
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Temp. mínima} & \textbf{Pontos usados} & \textbf{Erro em $h$} & \textbf{$R^2$} \\
\midrule
0~K (sem corte) & 49 de 49 & 99{,}0\% & 0{,}062 \\
1800~K (padrão) & 29 de 49 & 5{,}6\% & 0{,}997 \\
2000~K & 23 de 49 & 3{,}1\% & 0{,}999 \\
\bottomrule
\end{tabular}
\end{center}

\begin{aviso}
\textbf{Ajustando o corte.} O padrão de 1800~K funciona bem para a montagem
típica. Se o seu filamento ou LED forem diferentes, use o gráfico como guia:
suba o valor até que os pontos cinza cubram toda a região onde a curva de
fotocorrente está achatada no ruído. Colocar 0 desliga o corte e usa tudo ---
útil para ver o problema acontecer, não para medir.
\end{aviso}
...
\begin{description}[style=nextline,leftmargin=0.6cm]
  ...
  \item[O resultado de $h$ saiu muito distante do esperado]
    Verifique quantos pontos entraram na regressão, no painel de status. Se
    forem quase todos os coletados numa varredura que começou em tensão baixa,
    aumente a \campo{Temp. mínima p/ regressão} --- veja a
    seção~\ref{sec:faixa}. Se o $R^2$ estiver alto e a contagem parecer
    razoável mas $h$ ainda estiver deslocado, o problema tende a ser de
    calibração: a resistência a frio (com sua temperatura ambiente), os
    coeficientes escolhidos e o comprimento de onda do LED são os parâmetros
  ...
  \item[Temperaturas absurdas em tensão baixa]
    ...
    77~K). Isso é esperado e não indica falha do equipamento: esses pontos
    aparecem em cinza no gráfico e são automaticamente excluídos da regressão.
