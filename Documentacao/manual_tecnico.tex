...
\begin{nota}
\textbf{Limite conhecido.} Quando a resistência medida fica \emph{abaixo} de
$R_0$ --- o que acontece em tensões próximas de zero --- a
...
início das varreduras completas.
\end{nota}
...
O filtro atual é um limiar fixo sobre a fotocorrente:
...
O efeito desse critério pode ser medido. Numa varredura completa de 0 a 12~V
com 49 pontos, gerada pela bancada simulada:
...
$I > 1$~nA (limiar atual) & 49 & $6{,}9 \times 10^{-36}$ & 99{,}0\% & 0{,}062 \\
$I > 25$~nA & 27 & $6{,}32 \times 10^{-34}$ & 4{,}6\% & 0{,}999 \\
$T > 1800$~K & 29 & $6{,}31 \times 10^{-34}$ & 4{,}8\% & 0{,}999 \\
...
\begin{nota}
\textbf{Leitura do quadro.} O limiar de 1~nA \emph{não descarta nenhum ponto}:
o próprio piso do multímetro é da ordem de 4~nA, portanto maior que o limiar.
Um corte coerente com o instrumento precisa vir do modelo de erro dele --- a
especificação de exatidão na faixa de 100~$\mu$A tem um termo de fundo de
25~nA --- e não de um número escolhido à mão. Essa correção é trabalho
planejado, e a bancada simulada existe justamente para poder verificá-la.
\end{nota}
...
\begin{nota}
\textbf{Sobre a resistência calculada.} O software usa a tensão
\emph{programada} para calcular $R = V/i$, não a tensão lida de volta da fonte.
A leitura de retorno é mais exata que o valor programado, e a medição é a dois
fios, então a resistência dos cabos entra no valor de $R$. Os drivers já
expõem um método de leitura de tensão preparado para essa correção.
\end{nota}
...
\texttt{Resistencia\_Ohms} & $\Omega$ & Calculada, $V/i$ \\
...
\begin{nota}
Este formato é estável e há dados históricos gravados nele. Alterações de
esquema devem preservar a leitura dos arquivos antigos.
\end{nota}
